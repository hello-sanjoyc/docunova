...
function TwoFactorStep({ twoFactorToken, onSuccess, onBack }: TwoFactorStepProps) {
    ...
    <div className="w-full max-w-sm bg-cream border border-border rounded-2xl p-8 shadow-[0_4px_24px_rgba(15,14,12,0.06)]">
      <div className="flex items-center gap-3 mb-6">
        <div className="w-9 h-9 rounded-full bg-amber/10 flex items-center justify-center shrink-0">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#C8852A" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
            <rect x="5" y="11" width="14" height="10" rx="2" />
            <path d="M8 11V7a4 4 0 018 0v4" />
          </svg>
        </div>
        <div>
          <h1 className="text-[16px] font-semibold text-ink leading-tight">Two-factor authentication</h1>
          <p className="text-[13px] text-muted">Enter the 6-digit code from your app.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} noValidate>
        ...
        {error && (
          <p role="alert" className="text-[13px] text-danger mb-4 bg-danger-light rounded-lg px-3 py-2">
            {error}
          </p>
        )}
...
function LoginPageInner() {
  ...
  const [fields, setFields] = useState<Fields>({ email: prefilledEmail, password: "" });
  ...
  return (
    <div className="min-h-screen bg-cream flex flex-col items-center justify-center px-6 py-16">
      ...
      <div className="w-full max-w-sm bg-cream border border-border rounded-2xl p-8 shadow-[0_4px_24px_rgba(15,14,12,0.06)]">
        <h1 className="text-2xl font-light tracking-tight text-ink mb-1">Welcome back</h1>
        <p className="text-[14px] text-muted mb-8">Sign in to your DocuNova account.</p>
        ...
        <button
          type="button"
          onClick={() => { window.location.href = getGoogleOAuthUrl("login", nextPath); }}
        ...
        <form onSubmit={handleSubmit} noValidate>
          <div className="space-y-4 mb-5">
            <div>
              <label htmlFor="email" className="block text-[12px] font-medium text-ink mb-1.5">Email</label>
              <input
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label htmlFor="password" className="text-[12px] font-medium text-ink">Password</label>
                <Link href="/forgot-password" className="text-[12px] text-muted hover:text-amber transition-colors">
                  Forgot password?
                </Link>
              </div>
              <input
                ...
                onBlur={handleBlur} className={inputClass("password")}
              />
        ...
        <div className="flex justify-center mt-4">
          <Link href="/" className="flex items-center gap-1.5 text-[13px] text-muted hover:text-ink transition-colors">
